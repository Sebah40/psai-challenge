...
## Qué encontré
- Bugs en los endpoints y cosas raras en los datos: los detallo abajo en "Qué cambié y por qué", cada uno con qué estaba mal, cómo me di cuenta y cómo lo resolví.
...
## Más allá del brief (fuera de las 2-3h)
...
* Loaders para que la UI no parezca colgada al cambiar de filtro o paginar: skeleton en las cards, spinner en los charts y un overlay con spinner sobre la tabla mientras cargan. Los aciertos de cache no muestran loader (son instantáneos), el spinner sale solo cuando se va al server.

* Cambié los filtros de canal y marca de <datalist> a un combobox propio (componente Combobox). El datalist nativo filtra las opciones del lado del navegador de forma sensible a acentos, así que al escribir "tio" ocultaba "Tío Nacho" aunque el server la devolviera. El combobox muestra los resultados del server tal cual, con botón de limpiar y selección por click o Enter.
